...
(function ($) {
    ...
    TimeSlider.prototype.add_cell = function(timecell) {
        ...
        if (! this.$ruler.find('#' + timecell['_id']).length) {
            ...
            this.$ruler.append(
                ...
                '<div id="t' + timecell['_id'] + '" p_id="' + timecell['_id'] + '" class="timecell-event' + t_class + '" style="' + style + '">'+
                `P${timecell['_period']}`+
                '</div>'
            );
            this.$prompts.append(
    ...
    TimeSlider.prototype.set_time_cell_position = function(diff_x) {
        ...
        var period = this.time_cell_selected.element.attr('_period');
        var timecell = {
        ...
        if (this.time_cell_selected.l_prompt && this.time_cell_selected.r_prompt) {
            ...
            var d_start =this.timestamp_to_date( timecell['start']);
            var d_stop =this.timestamp_to_date( timecell['stop']);
                    ...
                    type:"POST",
                     data:({[csrfName]: csrfHash,'id':id,'type':'move','data':`${d_start},${d_stop}`}),
                    dataType:'JSON',
             });
        }
        // resize left border
            ...
            if (typeof this.options.on_resize_timecell_callback == 'function') {
                ...
                $.ajax({
                    url:base_url+"admin/masterscheduler/edit_slider_period ",
                    type:"POST",
                     data:({[csrfName]: csrfHash,'id':id,'type':'left','data':d_start}),
                    dataType:'JSON',
             });
        ...
        // resize right border
            ...
            var d_stop =this.timestamp_to_date( timecell['stop']);
                $.ajax({
                    url:base_url+"admin/masterscheduler/edit_slider_period ",
                    type:"POST",
                     data:({[csrfName]: csrfHash,'id':id,'type':'right','data':d_stop}),
                    dataType:'JSON',
             });
